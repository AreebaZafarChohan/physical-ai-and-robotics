...
class AgentService:
    ...
    async def get_response(self, query: str, selected_text: Optional[str] = None) -> str:
        """
        ...
        if selected_text and selected_text.strip():
            ...
            logger.info(f"AgentService received query with selected text: '{full_query}'")
        else:
            full_query = query
            logger.info(f"AgentService received query: '{query}'")

        try:
            # The agent's run method internally handles tool invocation and LLM reasoning
            # based on its system prompt and attached tools.
            agent_response = await self.rag_agent.run(full_query)
            ...
            if agent_response.strip() == FALLBACK_RESPONSE:
                logger.warning(f"Agent responded with fallback for query: '{full_query}'")

            return agent_response
        except Exception as e:
            logger.error(f"Error during agent chat for query '{full_query}': {e}", exc_info=True)
    ...
    async def chat_with_agent(self, user_query: str) -> str:
        ...
        Handles the conversation with the RAG agent, including tool invocation and response generation.
        """
        logger.info(f"AgentService received chat query: '{user_query}'")
        return await self.get_response(user_query)
